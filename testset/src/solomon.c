...
static int pp[mm + 1] = {
    1, 0, 1, 1, 1, 0, 0, 0, 1}; /* specify irreducible polynomial coeffts */
   ...
   can be returned as error flags to the calling routine if desired.   */
{
  ...
  if (syn_error) /* if errors, try and correct */
  {
    ...
    /* compute the error location polynomial via the Berlekamp iterative
       algorithm, following the terminology of Lin and Costello :   d[u] is the
       'mu'th discrepancy, where u='mu'+1 and 'mu' (the Greek letter!) is the
       step number ranging from -1 to 2*tt (see L&C),  l[u] is the degree of the
       elp at that step, and u_l[u] is the difference between the step number
